...
def find_candidate_page_and_coords(image_files, winner_name):
    """
    全ページの画像リストをGeminiに渡し、ターゲットがいるページ番号と座標を特定する
    """
    # ページ番号を付与してプロンプトを作成
    prompt = f"""
...
def crop_by_gemini_coords(pdf_path, page_num, coords, winner_name):
    ...
    new_ymin = max(0, ymin - 50) # 上に5%余裕
    new_ymax = min(1000, ymax + 50) # 下に5%余裕
    # ------------------
    ...
    output_path = f"temp_cropped_{page_num}.png"
    pix.save(output_path)
    doc.close()
    return output_path

def process():
    test_files = [Path("/Users/home/Minerva-1/data/raw_pdf/2026/shu/tokyo/tokyo-02.pdf")]
    ...
    for pdf_path in test_files:
        ...
        response = client.models.generate_content(
            model='gemini-3.1-flash-lite-preview', # ← ここを2.0に修正
