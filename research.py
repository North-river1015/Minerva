...
def overlapping (district, num):
    ...
    with open(manifesto_file, "r", encoding="utf-8") as f:
        ...
        同じ内容の公約がJsonに重複して存在する場合、is_verlappingはTrueです。存在しない場合、is_overlappingはFalseです。
        ...
        if is_overlapping:
            ...
            overlapping_prompt= f"""JsonファイルのManifestoの欄を確認し、重複している公約を探してください。
            すでに抽出した項目と内容が重複（例：「憲法への自衛隊明記」と「憲法改正による自衛隊明記」）している場合、
            より具体的で、ソースが最新のものを1つだけ残し、他は削除してください。重複していない部分やManifesto以外の部分は一切変更しないでください。

            元と同じように、Jsonデータの形で出力してください。
            """
            response = safe_generate_content(
                client=client,
...
def get_manifesto(district,winner,num):
    ...
    prompt= """ページが2026年以前のものであれば、Falseと返してください。2026年であれば、入力されたページの全文を読み込み、公約について少しでも書いているページか判断してください。
