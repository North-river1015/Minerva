...
def research(district,winner,num):
    ...
    具体的であれば、たとえ小さな項目であっても全てリストアップしてください。最低でも20〜30件の公約を抽出することを目指してください。複数ページから政策を取り出すだけで、満足しないでください。しつこく執念深く検索を続けてください。
     ...
     # Task
    1. Google Search Groundingを使用し、after:2025-03-30 の対象者の公式ウェブサイトや公式ブログ、報道機関によるアンケートなどを分析し、公約を漏れなく抽出してください。「公約」とのページ以外にも、ブログの欄などに政策がある可能性もあるため、公式ウェブサイトのリンクから政策がある可能性がある点をクロールしてください。サイトマップを執念深く確認してください。
    ...
    教育・子育て・デジタル
    厚生労働・医療・地域課題 
    の４つのカテゴリごとに個別に after:2025-03-30 の期間指定で執念深い検索を行い、各カテゴリ最低5件以上の政策を抽出してください。
...
textでも要約をせず、必ず出典元から語尾も含めて抜き出してください。 
    ...
    out_file = OUT_DIR / f"3improved?{district}-{num:02d}-prev.json"
...
def filter(district,winner,num):
    ...
    deep_research_output=open(f"output/research/3improved?{district}-{num:02d}-prev.json").read()
...
3. 【公共性】: 特定地域への利益誘導ではなく、全国的な制度変更や法的措置であること。
    ...
    4. **明確な賛否表明**: アンケート等での「選択的夫婦別姓への反対/賛成」「消費税維持」など。
上記１−４はコミットメントとして扱う。

# 排除基準（これらは公約ではないので削除せよ）
...
# 思考の出力例(reasonの欄に出力)
- 「所得税減税」→ 却下。理由：どの変数を動かすか不明なため。
- 「住宅ローン減税の床面積要件緩和」→ 採択。理由：床面積という単一変数の操作。
...
# 判定の鉄則：変数の特定性（パラメータ・レベルの検証）
「1円でも動かせば達成」と言い逃れができる曖昧な記述はすべて排除してください。
操作する「制度のレバー（変数）」が単一に特定されているものだけが「公約」です。

# 【合格・不合格の境界線（Few-Shot）】
以下の実例を判断の絶対基準としてください。
...
"""
    ...
    out_file = f"output/data/diditimprove6.json"
    with open(out_file, "w", encoding="utf-8") as f:
        f.write(final_json)            
        print("保存しました。")
